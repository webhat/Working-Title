 ...
 */
jQuery.log = function(message, dumpObject){
	...
	if( window.DEBUG ){
		...
		var str = message;
		if (typeof(message) == 'object' && dumpObject) {
			var str = '';
			for (var name in message) {
				try {
					str += ' ' + name + (typeof(message[name]) == 'function' ? '()' : '="' + message[name] + '"');
				}
				catch (ex) {
					str += ' ' + name + '="???"';
				}
		...
		if( !('firebug' in console) ){
			if( typeof(message) == 'object' ){
				if (message.nodeName && !dumpObject) {
